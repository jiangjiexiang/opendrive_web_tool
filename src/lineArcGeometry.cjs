...
function buildLineArcGeometryFromPoints(points) {
  ...
  if (pts.length === 3) {
    const arc = arcGeometryFromThreePoints(pts[0], pts[1], pts[2], 0);
    if (arc) {
      return {
        geometry: [{
          ...arc,
          s: 0,
          length: Number(Number(arc.length || 0).toFixed(6))
        }],
        length: Number(Number(arc.length || 0).toFixed(6))
      };
    }
  ...
  if (geometry.length >= 3) {
    if (String(geometry[0].type).toLowerCase() !== 'line') return null;
    if (String(geometry[geometry.length - 1].type).toLowerCase() !== 'line') return null;
  }
