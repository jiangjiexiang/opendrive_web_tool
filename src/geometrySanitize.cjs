...
const { buildGeometryFromPoints } = require('./vtsRules');
const { buildLineArcGeometryFromPoints } = require('./lineArcGeometry.cjs');
...
function sampleArcPoints(segment, step = 0.4) {
  ...
  if (Math.abs(k) < 1e-10) {
    for (let i = 1; i <= n; i += 1) {
      const s = (len * i) / n;
      points.push({
        x: x0 + Math.cos(hdg0) * s,
        y: y0 + Math.sin(hdg0) * s
      });
    }
    return points;
  }
...
function sampleSegmentToPoints(segment, step = 0.4) {
  ...
  if (type === 'line') {
    ...
    return [
      ...
      {
        x: Number(segment.x || 0) + Math.cos(hdg) * len,
        y: Number(segment.y || 0) + Math.sin(hdg) * len
      }
    ];
  }
...
/**
 * Keep only line / arc / spiral. Unsupported types (e.g. paramPoly3) are
 * resampled and refit as line+arc (or line-only fallback).
 */
function sanitizeGeometryTypes(rawGeometry) {
